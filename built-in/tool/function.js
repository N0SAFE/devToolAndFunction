...
export async function getFromFile(...array) {

    if (sessionStorage.baseURL == undefined || sessionStorage.baseURI == undefined) {
    ...
    // console.log(sessionStorage.baseURL)
    ...
    await array.asyncForEach(async function(paths, param) {
        ...
        await paths.asyncForEach(async function(path) {
            ...
            if ((await fetch(sessionStorage.baseURL + "built-in/php/verifyFileExists.php", { method: "POST", body: path })
                    .then((response) => {
                        if (response.ok) {
                ...
                promise.push(
                    fetch(new URL(path, sessionStorage.baseURL), { method: param.method, body: param.data })
                    .then((response) => {
                        if (response.ok) {
                            return response.text();
                        } else {
                ...
                if (param.showError) {
                    console.error("function.js:137 POST " + new URL(path, sessionStorage.baseURL) + " 404 (Not Found)")
...
export async function getProcessTime(callback, args, treatementProcess) {
...
export function getRandomInt(max, min = null) {
    if (min == null) {
...
export async function getArrayFunction() {
    return await
    import ("./tool/    .js").default
}
export async function getObjectFunction() {
    return await
    import ("./tool/ObjectFunction.js").default
}
